...
function Util:GetProductPrice(productId)
	...
	if success then
		...
		warn("Error getting price for Morph product ".. tostring(productId)..": ".. tostring(err))
	end
	return price
end
...
function Util:ServerGetTorsoFromPlayer(player)
	local human = nil
	if player then
		local character = player:WaitForChild("Character", 2)
		if character then
...
-- Returns true if player is on spawn location
function Util:ServerIsPlayerOnSpawnLocation(player)
	local torso = Util:ServerGetTorsoFromPlayer(player)
	if torso then
		local spawnLoc = workspace.SpawnLocation
		if (torso.Position - spawnLoc.Position).magnitude < 9 then
			return true
		end
	end
	return false
